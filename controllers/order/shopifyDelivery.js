...
async function computeParcelStats(order) {
  let pieces = 0;
  let totalWeightKg = 0;
  const itemIds = order.products.map(p => p.item);
  const items = await Item.find({ _id: { $in: itemIds } }).select("_id weight title");
  const weightById = new Map(items.map(it => [String(it._id), Number(it.weight || 0)]));
  for (const p of order.products) {
    const qty = Number(p.quantity || 0);
    pieces += qty;
    const w = weightById.get(String(p.item)) || 0;
    totalWeightKg += w * qty;
  }
...
export async function createConsignmentAutoAWB(orderId) {
  const order = await Order.findById(orderId)
    ...
    .populate("products.item", "title")
    .populate("sellerStore", "name phone");
  ...
  const recPhone = order.shippingDetails?.phone || order.customer?.phone || order.sellerStore?.phone || "";
